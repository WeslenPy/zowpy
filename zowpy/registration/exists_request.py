"""
Async Exists Request - Requisição de existência assíncrona.
"""
...
class AsyncExistsRequest(WARequest):
    """Requisição de existência assíncrona."""

    def __init__(self, 
                 phone_number: str, 
                 config: Optional[Config | AsyncProfile] = None, 
        ...
        self.pvars = ["status", "reason", "sms_length", "voice_length", "result","param", "login", "type",
                      "chat_dns_domain", "edge_routing_info","appeal_token","violation_type","in_app_ban_appeal",
        ...
        if self.env.deviceEnv.getOSName() in ["Android","SMBA"]:
            ...
            self.addParam("pid",random.randint(10000, 99999))
