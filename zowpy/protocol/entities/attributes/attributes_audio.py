"""
AudioAttributes - Atributos específicos para áudios.

Baseado em zowsuplib/yowsup/layers/protocol_messages/protocolentities/attributes/attributes_audio.py
"""
...
from ....utils.media_tools import AudioTools, normalize_file_path_or_url


class AudioAttributes:
    """
    Atributos específicos para mensagens de áudio.
    
    Contém informações sobre duração, push-to-talk e waveform.
    """
    ...
    def __init__(
        ...
        """
        Inicializa AudioAttributes.
        
        Args:
            downloadablemedia_attributes: Atributos de mídia downloadable
            seconds: Duração do áudio em segundos
            ptt: Se é uma mensagem push-to-talk (voice message)
            streaming_sidecar: Dados de streaming sidecar (opcional, bytes)
            waveform: Dados de waveform para visualização (opcional, bytes, tipicamente 100 bytes)
        """
    ...
    def __str__(self):
        ...
        if self.waveform is not None:
            attrs.append(("waveform", f"[{len(self.waveform)} bytes]"))
    ...
    async def from_filepath(
        ...
        audio_properties: Optional[int] = None,
        ptt: bool = False,
        streaming_sidecar: Optional[bytes] = None,
        waveform: Optional[bytes] = None
    ) -> 'AudioAttributes':
        """
        Cria AudioAttributes a partir de arquivo local.
        ...
        Args:
            ...
            audio_properties: Duração em segundos - se None, será detectado automaticamente
            ptt: Se é push-to-talk (padrão: False)
            streaming_sidecar: Dados de streaming sidecar (opcional)
            waveform: Dados de waveform (opcional) - se None e ptt=True, será gerado automaticamente
    ...
    async def from_url(
        ...
        """
        Cria AudioAttributes a partir de URL.
