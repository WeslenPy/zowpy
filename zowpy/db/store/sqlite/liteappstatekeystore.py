...
from ....protocol.historysync.attributes import (
...
class LiteAppStateStore(AppStateStore):
    ...
    def getAppStateKey(self, key_id):
        # Corrigido: SELECT deve incluir key_data e timestamp também
        q = "SELECT key_id, key_data, timestamp FROM app_state_keys WHERE key_id = ?"
        c = self.dbConn.cursor()
        c.execute(q, (key_id, ))
